...
Outputs: `attempts` (total command runs), `recovered` (`true` only if a retry succeeds), and `category` (last valid failure category). Supported retry categories are `network`, `runner`, `test_flake`, and `dependency`.
...
There is no live Jev integration test yet. Before relying on the retry decisions, evaluate them in your own CI against known transient and deterministic failures.
